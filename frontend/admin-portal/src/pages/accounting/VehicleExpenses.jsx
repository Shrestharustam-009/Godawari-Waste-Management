    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 transition-colors duration-200 rounded-xl shadow-xl w-full max-w-lg flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Category Name (e.g. Fuel, Repairs)</label>
      ...
      <div className="bg-white dark:bg-slate-800 transition-colors duration-200 rounded-xl shadow-xl w-full max-w-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Vehicle</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Fleet Cost Bucket (Category)</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Transaction Date</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Notes / Invoice Number</label>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Vehicle & Fleet Expenses</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Specialized ledger for tracking fleet operational costs.</p>
        </div>
              ...
              {filteredEntries.length > 0 ? (
                filteredEntries.map(tx => (
                  <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900/50/80 transition-colors bg-white dark:bg-slate-800 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4 font-bold text-slate-700 dark:text-slate-300">#{tx.vehicleId}</td>
                    <td className="px-6 py-4 text-slate-900 dark:text-white font-medium">{tx.registrationNumber}</td>
                ...
                <tr><td colSpan={6} className="text-center py-16 text-slate-400 dark:text-slate-500 font-medium bg-white dark:bg-slate-800 transition-colors duration-200">No vehicle expenses found.</td></tr>
              )}
            </tbody>
          </table>
          </div>
